...
function print_usage() {
    ...
    echo "  -l, --add-langvar   Add env. variable \$GTLANG_smn to login script."
...
if test x$langvar = xlangvar ; then
	if test x$LOGINFILE = x ; then
		...
		if test x$GTLANG_smn = x$LANGDIR ; then
			echo "\$GTLANG_smn already defined."
		else
			# Already defined with a different value:
			if test x$GTLANG_smn != x ; then
				renew=renew
				OLDLANGDIR=$GTLANG_smn
			fi

			# Add the variable to the login script:
			cp $LOGINFILE $LOGINFILE.gtbackup.${DATE}-${TIME}
			echo "export GTLANG_smn=$LANGDIR" >> $LOGINFILE
			source $LOGINFILE

			# Feedback depending on whether it was added or redefined:
			if test x$renew = xrenew ; then
			 echo "The env. variable \$GTLANG_smn has been redefined in"
			 echo "$LOGINFILE. The old value $OLDLANGDIR is no longer in use."
			else
			 echo "The env. variable \$GTLANG_smn has been added to $LOGINFILE."
...
# the current dir:
if   test x$GTLANG_smn = x ; then
	echo "WARNING: The variable \$GTLANG_smn has not been defined. You will not"
	...
	echo "$0 -l"
	echo
elif test x$GTLANG_smn != x$LANGDIR ; then
	echo "WARNING: The variable \$GTLANG_smn has the value:"
	echo "  $GTLANG_smn"
