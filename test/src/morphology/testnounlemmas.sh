# Kommando når man er i smn: sh test/src/morphology/testnounlemmas.sh
...
grep ";" src/morphology/stems/nouns.lexc | egrep -v "^(\!|\@|<)" | egrep -v '(CmpN/Only|ShCmp|RCmpnd| Rreal | R | Rnoun |\+V\+|\+Err\/Lex)' | sed 's/% /€/g' | sed 's/%:/¢/g' |  tr ":+" " " | cut -d " " -f1 | tr -d "%" | tr "€" " " | tr "¢" ":" | sort -u > nouns

# Legg substantivet viessu til lemmaer som krever sammensetning, lagres i nouns
grep ";" src/morphology/stems/nouns.lexc | egrep -v "^(\!|\@|<)"  | egrep -v '(ShCmp|\+Err\/Lex)' |  egrep "( R | Rreal | Rnoun)" | tr ":+" " " | cut -d " " -f1 | sed 's/$/viessu/' >> nouns
...
grep ";" src/morphology/stems/nouns.lexc | egrep -v "^(\!|\@|<)"  | egrep -v '(ShCmp|\+Err\/Lex)' | egrep "( R | Rreal | Rnoun)" | tr ":+" " " | cut -d " " -f1 | sed 's/$/+N+Cmp#viessu+N+Sg+Nom/' | $LOOKUP src/generator-gt-norm.xfst | cut -f2 | grep -v '^$' >> analnouns
...
grep ";" src/morphology/stems/nouns.lexc | egrep -v "^(\!|\@|<)" | grep -v '(ShCmp|\+Err\/Lex)' | grep "RHyph" | tr ":+" " " | cut -d " " -f1 | sed 's/$/+N+Cmp-#viessu+N+Sg+Nom/' | $LOOKUP src/generator-gt-norm.xfst | cut -f2 | grep "\-" | cut -d "-" -f1 >> analnouns
